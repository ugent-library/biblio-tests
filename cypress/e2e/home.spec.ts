describe("The Home page", () => {
  it("should display the highlighted publications and datasets", () => {
    ...
    cy.contains("New open access publications and datasets").should(
      "be.visible",
    );

    cy.get<HTMLAnchorElement>(
      ".content-header-home ul:not(.list-unstyled) > li > a",
    )
  ...
  it("should link the publications and datasets search pages", () => {
    ...
    cy.contains(
      ".content-header-home",
      "View all open access publications and datasets",
    ).should("be.visible");

    cy.contains(".content-header-home a", "publications").click();
    cy.location("pathname").should("eq", "/publication");
    cy.get("ul[data-facet=type]")
      .find(":checkbox[value=researchData]")
      .should("not.exist");
